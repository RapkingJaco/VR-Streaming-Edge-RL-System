...
def plot_baseline_chart():
    ...
    if file_path is None:
        print("❌ 找不到 Baseline_*.csv 檔案，請確認是否已完成錄製並放置於正確資料夾。")
        return

    print(f"📂 鎖定基準線檔案：{file_path}")
    # 輸出檔名標註為 Baseline_Analysis
    ...
    try:
        ...
        # ==========================================
        # 第 2 層：Load Ratio (專家規則分配結果)
        # ==========================================
        color_ratio = 'tab:green'
        ax2.plot(df['Elapsed(s)'], df['LoadRatio'], color=color_ratio, label='Rule-Based Load Ratio', linewidth=2.5)
        ...
        ax3.set_ylim(0, 30) # 稍微放寬一點點確保抖動不會出鏡
        ...
        ax3_right.set_ylim(0, 10) # 配合網路風暴強度
        ...
        print(f"✅ 成功！基準線分析圖已儲存為: {output_image}")
        plt.show()
